...
test 'empty items are parsed correctly', (t) ->
    parser = new ItemParser

    parser.on 'error', (err) -> t.fail()
    parser.on 'end', (earnings) ->
      t.deepEqual earnings, []
      t.end()
...
test 'one item is parsed correctly', (t) ->
  parser = new ItemParser
...
test 'three items are parsed correctly': (t) ->
  parser = new ItemParser
